...
function chess!(r)
    v, g = corner_put_and_count!(r, Sud, West)
    if mod(v, 2) == mod(g, 2)
        putmarker!(r)
    end
    go_move!(() -> isborder(r, Nord),r, Ost, Nord)
    corner_put_and_count!(r, Sud, West)
    nmove!(r, Nord, v)
    nmove!(r, Ost, g)
end

function go_move!(st_cond, r, tec, osn)
    while !st_cond()
...
function corner_put_and_count!(r, side1, side2)
